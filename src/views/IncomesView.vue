...
import { Money, HomeFilled } from '@element-plus/icons-vue'
...
const columns = ref([
  ...
  { key: 'amount', label: 'Сумма', type: 'number', sortable: true, width: '120px' },
  { key: 'source', label: 'Источник', sortable: false },
  { key: 'category', label: 'Категория', sortable: false, width: '150px' },
  { key: 'description', label: 'Описание', sortable: false },
])

// Числовые поля для графика
const numericFields = ref([
  { key: 'amount', label: 'Сумма дохода' },
  { key: 'net_amount', label: 'Чистый доход' },
...
const loadData = async () => {
  ...
  try {
    ...
    ElMessage.error('Ошибка загрузки данных доходов')
    tableData.value = []
    chartData.value = []
  } finally {
    loading.value = false
